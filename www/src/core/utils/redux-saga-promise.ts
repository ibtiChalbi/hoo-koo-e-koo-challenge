...
export type PromiseAction<P = void, A = void, B = void> = PayloadAction<
  P,
  string,
  PromiseActionMeta<A, B>
>;

export function createPromiseAction<P = void, A = void, B = void>(
  prefix: string
): PromiseActionCreatorWithPayload<P, A, B> {
  const resolveAction = createAction<PrepareAction<A>, string>(
    `${prefix}.RESOLVED`,
    (payload) => ({ payload })
  );
  const rejectAction = createAction<PrepareAction<B>>(
    `${prefix}.REJECTED`,
    (payload) => ({ payload })
  );

  const triggerAction = createAction<PreparePromiseAction<P, A, B>>(
    `${prefix}.TRIGGER`,
    (payload: P) => ({
      payload,
      meta: {
...
export function* resolvePromiseAction<P = void, A = void, B = void>(
  action: PromiseAction<P, A, B>,
  value: A
): Generator {
...
export function* rejectPromiseAction<P = void, A = void, B = void>(
  action: PromiseAction<P, A, B>,
  value: B
): Generator {
...
const isTriggerAction = (action: PayloadAction<any, string, any>) =>
  !!action.meta?.promise?.resolveAction;

export const promiseMiddleware: Middleware<unknown, AppState> =
  () => (next) => (action: PayloadAction<any, string, any>) => {
